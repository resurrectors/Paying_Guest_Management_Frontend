...
export default function Nrooms() {
    ...
    const getAllRooms = async () => {
        const allRooms = await retrieveRooms();
        if (allRooms) {
    ...
    function handleDelete(roomId) {
        ...
        deleteRoom().then(() => getAllRooms()).catch(() => console.log("Booking exist cannot delete"));
    }
    ...
    return (
        <div className='container-fluid'>
                    ...
                    <div id="carouselExampleControls" className="carousel slide" data-bs-ride="carousel">
                        <div className="carousel-inner">
                            <div className="carousel-item active">
                                <img src="https://media.istockphoto.com/videos/animated-retro-banner-video-overlay-in-white-video-id1308987644?s=640x640" className="d-block w-100" style={{'height':'250px'}} alt="..." />
                            </div>
                            {
                                bimages.map(i =>
            ...
            <div className='row'>
                ...
                <div className='col-8'>
                    <div>
                        <h5 className="card-title" style={{ 'marginBottom': '20px' }}>Rooms in building {buildingId}</h5>
                        <hr />
                        <Link  to={`/owner/building/${buildingId}/addroom`} className="btn btn-outline-success text-right" style={{ 'marginBottom': '20px' }}>Add New Room</Link>
                        &nbsp;&nbsp;
                        <Link className="btn btn-outline-info" style={{ 'marginBottom': '20px' }} to={`/image/building/${buildingId}`}>Upload Building Images</Link>
                    </div>
                    {
                        rooms.map(r =>
                                ...
                                <div className="row g-0">
                                    <div className="col-12">
                                        <div className="card-body">
                                            <h5 className="card-title">Rent Per Day : {r.rentPerDay}</h5>
                                            {/* <p className="card-text">This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.</p> */}
                                            <p className="card-text">
                                                <div style={{ 'textAlign': 'right' }}>

                                                    <Link className="btn btn-outline-info" to={`/owner/room/bed/${r.id}`}>View Beds</Link>
